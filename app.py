...
class Venue(db.Model):
    ...
    #TODO: implement any missing fields here, as a database migration using flask-Migrate
...
# this is used to format datetime
...
# this is the index controller page
@app.route('/')
def index():
...
def venues():
  ...
  data = []
  # This is to get all venues
  venues = Venue.query.all()

  # I reconmmend we use set so there are no duplicate venues
  locations = set()

  for venue in venues:
    # This is to add city / state tuples
    locations.add((venue.city, venue.state))

  # This is for each unique city / state, add veneus
  ...
  for venue in venues:
    ...
    # This is to get current date to filter num_upcoming_shows
...
# This is to show a specific venue
...
def create_venue_submission():
  ...
  try:
    # This is to get form data and create
    form = VenueForm()
    ...
    # This is to commit session to database
    db.session.add(venue)
    db.session.commit()
    # This is to flash success
...
def delete_venue(venue_id):
  ...
  try:
    # This is used to Get venue by ID
    venue = Venue.query.get(venue_id)
...
def search_artists(): 
  ...
  # This is used to filter artists by case insensitive search
